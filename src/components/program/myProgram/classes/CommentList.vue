...
import SubCommentList from "@/components/program/myProgram/classes/SubCommentList.vue";
...
const getCommentList = () => {
  const _url: string = "/api/v1/mainCommentList";
  const param: object = {
    eliId: prop.eliId,
    eccDivCd: prop.eccDivCd,
  };
...
const writeComment = () => {
  ...
  const _url = "/api/v1/writeMainComment";
  let param: object = {
    eliId: prop.eliId,
    eccDivCd: prop.eccDivCd,
  ...
  axios
    ...
    .then((response: AxiosResponse) => {
      ...
      if (catchData) {
        ...
        //alert("댓글 수정이 실패하었습니다");
      }
    })
    // eslint-disable-next-line no-unused-vars
    .catch((error: Error) => {
      //alert("댓글 수정이 에러하었습니다");
    });
};
</script>
